...
///
/// This a specific radio frequency used by a Bluetooth LE controller. This is used by test commands
/// to drive the radio to a specific frequency.
#[derive(Debug, Copy, Clone, Eq, PartialEq)]
...
impl Frequency {
    ...
    ///
    /// The input `mega_hz` must be between the frequencies [`MAX`] and [`MIN`]. The frequency
    /// values are also in increments of two megahertz so if an odd value is provided it is floored
    /// to the nearest even number.
    ///
    /// # Error
    /// If `mega_hz` is outside the valid range, the violated bound is returned as an error.
    ///
    /// [`MAX`]: Frequency::MAX
    /// [`MIN`]: Frequency::MIN
    ...
    /// Get the value for the `Frequency` as used within the HCI
    ///
    /// This is the value that gets transferred over the Host Controller Interface to represent the
    /// frequency value.
    ///
    /// # Note
    /// To convert this into a frequency it needs to be converted into a `usize` and then double the
    /// value and add '2402'. The calculated value is the frequency in megahertz.
...
macro_rules! make_interval {
        ...
        $micro_sec_conv:expr) =>
    {
        ...
        impl $name {
            ...
            const RAW_RANGE: crate::commands::le::IntervalRange<u16> = crate::commands::le::IntervalRange{
                low: $raw_low,
                hi: $raw_hi,
            ...
            {
                let duration_range = crate::commands::le::IntervalRange::<core::time::Duration>::from($name::RAW_RANGE);
...
interval!(ConnectionInterval, 0x0006, 0x0C80, ApiDef, 0x0006, 1250);
