    ...
    for HostChannelEnds<'a, TASK_COUNT, CHANNEL_SIZE, BUFFER_SIZE>
{
    ...
    where
        C: Into<Option<usize>>,
    {
        self.from_host_channel.take(front_capacity.into().unwrap_or_default())
    }
...
mod test {
    ...
    fn local_static_init_usize() {
        let _: LocalStackBufferedChannel<20, usize> = LocalStackBufferedChannel::new();
    }

    #[test]
    fn local_static_init_ref_mut_usize() {
        let _: LocalStackBufferedChannel<20, &mut usize> = LocalStackBufferedChannel::new();
    }

    #[tokio::test]
    async fn local_static_add_remove_usize() {
        let ls: LocalStackBufferedChannel<5, HciPacket<usize>> = LocalStackBufferedChannel::new();
    ...
    async fn local_static_add_remove_usize_single_capacity() {
        let ls: LocalStackBufferedChannel<1, HciPacket<usize>> = LocalStackBufferedChannel::new();
    ...
    async fn local_static_add_remove_byte_slice() {
        let l: LocalStackBufferedChannel<4, HciPacket<&[u8]>> = LocalStackBufferedChannel::new();
    ...
    async fn local_add_remove_array() {
        ...
        let l: LocalStackBufferedChannel<4, HciPacket<[usize; SIZE]>> = LocalStackBufferedChannel::new();
