...
fn check_generic_args<'a>(
    ...
    generic_args.any(|ga| match ga {
        syn::GenericArgument::Type(syn::Type::Path(syn::TypePath {
            ...
            .first()
            .map(|first| first.ident == bubble.generic_name.as_ref())
            .unwrap_or_default(),
        _ => false,
    })
}
...
pub(super) fn already_has_generic<'a>(
    ...
    params.any(|gp| match gp {
        syn::GenericParam::Type(syn::TypeParam { ident, .. }) => ident == bubble.generic_name.as_ref(),
        _ => false,
    })
}

pub(super) fn modify_path_segment(segment: &mut syn::PathSegment, bubble: &mut super::Bubble) -> ret!() {
    match &mut segment.arguments {
        syn::PathArguments::None => segment.arguments = syn::PathArguments::AngleBracketed(syn::parse_quote!(::<F>)),
...
pub(super) fn modify_where_clause(clause: &mut Option<syn::WhereClause>, bubble: &mut super::Bubble) -> ret!() {
    use std::mem::take;
