...
impl AccessValue for LocalHeartRateMeasurementArc {
    ...
    type Read<'a>
        = Pin<Box<dyn Future<Output = Result<Self::ReadGuard<'a>, bo_tie::host::att::pdu::Error>> + Send + Sync + 'a>>
    where
        Self: 'a;
    type WriteValue = ();
    type Write<'a>
        = std::future::Pending<Result<(), bo_tie::host::att::pdu::Error>>
    where
        Self: 'a;
...
impl AccessValue for ControlPoint {
    ...
    where
        Self: 'a;
    type Read<'a>
        = std::future::Pending<Result<Self::ReadGuard<'a>, bo_tie::host::att::pdu::Error>>
    where
        Self: 'a;
    type WriteValue = u8;
