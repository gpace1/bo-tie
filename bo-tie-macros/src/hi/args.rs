...
pub fn process_args(args: &syn::AttributeArgs) -> Result<AttributeConfig, String> {
    args.into_iter()
        .try_fold(AttributeConfig::None, |mut cfg, nested_meta| {
            match nested_meta {
...
fn process_named_val(named_val: &syn::MetaNameValue, cfg: &mut AttributeConfig) -> Result<(), String> {
    if named_val
        .path
        .get_ident()
        .map(|i| i == FLOW_CTRL_GENERIC)
        .unwrap_or_default()
    {
...
fn process_flow_ctrl_bounds(lit: &syn::Lit, cfg: &mut AttributeConfig) -> Result<(), String> {
    let bound_list = process_str_lit(lit)?.split('+').try_fold(Vec::new(), |mut v, s| {
